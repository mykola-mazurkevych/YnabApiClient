...
internal sealed class YnabV1AccountsApiClient(IHttpClientFactory httpClientFactory, Uri baseUri, string bearerToken)
    : YnabApiClientBase(httpClientFactory), IYnabV1AccountsApiClient
{
    ...
    public IYnabV1AccountApiClient this[Guid accountId] =>
        _accountClients.GetOrAdd(accountId, () => new YnabV1AccountApiClient(_httpClientFactory, _resourcesUri, accountId, bearerToken));

    public Task<AccountsResponse> GetAsync(CancellationToken cancellationToken = default) =>
        this.GetAsync<AccountsResponse>(_resourcesUri, null, bearerToken, cancellationToken);
